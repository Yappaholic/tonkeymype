...
Point CENTER = getScreenCenter(settings.SCREEN_WIDTH,settings.SCREEN_HEIGHT);
...
void Button::SetBackgroundColor(int red, int green, int blue, int alpha) {
  backgroundColor = {(Uint8)red, (Uint8)green, (Uint8)blue, (Uint8)alpha};
}

void Button::SetBackgroundColor(int red, int green, int blue) {
  backgroundColor = {(Uint8)red, (Uint8)green, (Uint8)blue, 255};
}
...
int Button::Render(SDL_Renderer* renderer) {
  ...
  //SDL_Color bgColor = {backgroundColor[0], backgroundColor[1], backgroundColor[2], backgroundColor[3]};
  if (surface == NULL) {
    surface = TTF_RenderText_Blended(Sans, text.c_str(), textColor);
  } 
  if (texture == NULL) {
  ...
  Button_rect.h = pointToPixel(24) * 1;
  Button_rect.w = pointToPixel(24) * 1;
